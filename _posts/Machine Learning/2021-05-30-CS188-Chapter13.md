...
![74c2def286635db980f08a29f1063bc](https://markdown-img-1304853431.file.myqcloud.com/20210530155955.jpg)
...
![](https://markdown-img-1304853431.file.myqcloud.com/20210530162342.jpg)
...
![789eb9ea9e270d655adce60dfe5763a](https://markdown-img-1304853431.file.myqcloud.com/20210530162436.jpg)
...
<mark>Probability statements are made with respect to a knowledge state, not with respect to the real world</mark>. When we say a student has a probability of $0.8$ to get low GPA, we are inference based on our knowledge state, not on the real world (as the student must either have high GPA or low GPA).

[^1]: Laziness: It's too much work to list the complete set of antecedents or consequent needed to ensure an exceptionless rule and too hard to use such rules
...
![b3d157353fb5ae7becc43a063d70e65](https://markdown-img-1304853431.file.myqcloud.com/20210530180125.jpg)
...
Probabilities like $P(X=0)$ is called **unconditional** or **prior probabilities** (先验概率). It refers to <mark>degrees of belief in propositions <em>in the absence of any other information</em></mark>. In most of the situations, we do have some known information before probabilistic query. Such information is called **evidence**, and the probabilities of event **given the evidence** is called **conditional** or **posterior probabilities** (后验概率).
...
Variables in probability theory are called **random variables**. Every random variable has a **domain** - the set of all valid value a random variable can be assigned to.
...
![ddaf7f6ffe19a8da0a87db7d48e3fef](https://markdown-img-1304853431.file.myqcloud.com/20210601192204.jpg)
...
By separating the independent random variables from all considered random variables, we can **factor** the full joint distribution into several independent joint distribution tables.
...
Such a probability distribution is called **Naïve Bayes** - it is "naïve" since it is often used (as simplification) in cases where the effect variables are *not* actually conditionally independent with each other.
